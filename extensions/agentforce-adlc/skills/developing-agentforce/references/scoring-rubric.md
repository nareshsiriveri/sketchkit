...
| FSM Architecture | 10 | Router-first architecture or verification gate pattern when required. Every subagent reachable. Every subagent has an appropriate transition path (direct subagent transition, router reclassification, or escalation). No orphan subagents. |
